...
async function wotGet(
  ...
  if (paymentHash) url.searchParams.set("payment_hash", paymentHash);

  const res = await fetch(url.toString());
  return (await res.json()) as Record<string, unknown>;
}

function formatL402(data: Record<string, unknown>): string {
  const l402 = (data as L402Response).protocols?.l402;
  if ((data as L402Response).status === "payment_required" || data.message?.toString().includes("Pay")) {
